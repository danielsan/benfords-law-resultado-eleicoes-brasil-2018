...
const parseLine = line => {const [date, time, year, x, , uf, , cep, city, a, b, c, role, candidate, votes] = line.replace(/"/g, '').split(';'), o = {date, time, year, x, uf, cep, city, a, b, c, role, candidate, votes};  return o;}
...
const init = async (filename) => {
    ...
    Object.keys(map).forEach((candidate) => {
      Object.keys(map[candidate]).forEach(number => map[candidate].total += map[candidate][number].value);
      Object.keys(map[candidate]).forEach(number => map[candidate][number].percentage = ((map[candidate][number].value / map[candidate].total) * 100).toFixed(2)+' %');
    });
